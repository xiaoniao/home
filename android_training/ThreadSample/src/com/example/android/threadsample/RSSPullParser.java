 ...
 * 解析RSS数据
 * 
 * RSSPullParser reads an RSS feed from the Picasa featured pictures site.
 * 
 * It uses several packages from the widely-known XMLPull API.
 * 
 */
public class RSSPullParser extends DefaultHandler {
	 ...
	 *            a helper class for sending status and logs
	 * @throws XmlPullParserException
	 *             defined by XMLPullParser; thrown if the thread is cancelled.
	 * @throws IOException
	 *             thrown if an IO error occurs during parsing
	 */
	public void parseXml(InputStream inputStream, BroadcastNotifier progressNotifier) throws XmlPullParserException, IOException {
		...
		// Loops indefinitely. The exit occurs if there are no more URLs to
		// process
		while (true) {
			...
			// If the current thread is interrupted, throws an exception and
			// returns
			if (Thread.currentThread().isInterrupted()) {
				...
				/*
				 * If this is the start of an individual item, logs it and creates a new ContentValues
				 */
				if (eventName.equalsIgnoreCase(ITEM)) {
					...
					if (eventName.equalsIgnoreCase(CONTENT)) {
						...
						// Stores the image URL and image name column names as
						// keys
						...
						// Stores the thumbnail URL and thumbnail name column
						// names as keys
					...
					// Gets the filename of the URL and puts it into the
					// ContentValues
			...
			/*
			 * If it's not an ITEM, and it is an END_TAG, and the current event is an ITEM, and there is data in the current ContentValues
			 */
			else if ((nextEvent == XmlPullParser.END_TAG) && (localXmlPullParser.getName().equalsIgnoreCase(ITEM)) && (mImage != null)) {

				// Adds the current ContentValues to the ContentValues storage
				mImages.add(mImage);

				// Logs progress
				progressNotifier.notifyProgress("Parsed Image[" + imageCount + "]:" + mImage.getAsString(DataProviderContract.IMAGE_URL_COLUMN));
