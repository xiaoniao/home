...
// 卧槽 继承自LinearLayout ， 接口和类都使用了泛型，值得推荐使用
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        if (refreshableView instanceof ViewGroup) {
            ...
            throw new UnsupportedOperationException("Refreshable View is not a ViewGroup so can't addView"); // 多多使用抛出异常
        }
    }
    ...
    public final boolean demo() {
        if (mMode.showHeaderLoadingLayout() && isReadyForPullStart()) { // 头部
            smoothScrollToAndBack(-getHeaderSize() * 2);
            return true;
        } else if (mMode.showFooterLoadingLayout() && isReadyForPullEnd()) { // 底部
    ...
    /** 重载触摸事件方法onInterceptTouchEvent **/
    @Override
    public final boolean onInterceptTouchEvent(MotionEvent event) {

        if (!isPullToRefreshEnabled()) { // 如果不允许下拉刷新就直接退出，不执行下方代码
            return false;
        }
    ...
    @Override
    public final void onRefreshComplete() { // 完成下拉刷新
        if (isRefreshing()) {
            setState(State.RESET);
        }
    }

    /** 重载触摸事件方法onTouchEvent **/
    @Override
     ...
     */
    public abstract Orientation getPullToRefreshScrollDirection(); // 抽象方法，由子类实现，获取展示方向
     ...
     */
    protected abstract boolean isReadyForPullEnd(); // 抽象方法，由子类具体实现
     ...
     */
    protected abstract boolean isReadyForPullStart(); // 抽象方法，由子类具体实现
    ...
    public static enum Mode {
         ...
         */
        DISABLED(0x0), // 移除所有的下拉刷新动作和正在下拉刷新操作
         ...
         */
        PULL_FROM_START(0x1), // 只允许从 'Refreshable' View 的开始处下拉刷新, 可以是顶部也可以是左侧，看它的方向了
         ...
         */
        PULL_FROM_END(0x2), // 只允许从 'Refreshable' View 的结束处下拉刷新, 可以是底部也可以是右侧，看它的方向了
         ...
         */
        MANUAL_REFRESH_ONLY(0x4); // 不需要手势操作，但允许手动设置刷新状态通过 setRefreshing()

        /**
         * @deprecated Use {@link #PULL_FROM_START} from now on.
         */
        public static Mode PULL_DOWN_TO_REFRESH = Mode.PULL_FROM_START; // 废弃使用，直接使用PULL_FROM_START ， 这就可以看出一个框架的设计历程啦

        /**
         * @deprecated Use {@link #PULL_FROM_END} from now on.
         */
        public static Mode PULL_UP_TO_REFRESH   = Mode.PULL_FROM_END;  // 废弃使用，直接使用PULL_FROM_END
         ...
         */
        static Mode mapIntToValue(final int modeInt) { // 通过xml设置Mode
            ...
            return getDefault(); // 如果失败使用默认 (注意这种思想)
        }

        static Mode getDefault() { // 返回默认方式
        ...
        Mode(int modeInt) {
            mIntValue = modeInt; // Mode计算值
        }

        /**
         * @return true if the mode permits Pull-to-Refresh
         */
        boolean permitsPullToRefresh() { // 返回是否允许下拉刷新
         ...
         */
        public boolean showHeaderLoadingLayout() { // 是否显示headerLoadingLayout
         ...
         */
        public boolean showFooterLoadingLayout() { // 是否显示footerLoadingLayout
            return this == PULL_FROM_END || this == BOTH || this == MANUAL_REFRESH_ONLY;
        }

        int getIntValue() { // 获得Mode的值
            return mIntValue;
        }
     ...
     * to append extra UI events (such as sounds).
     * 
     * See ( {@link PullToRefreshAdapterViewBase#setOnPullEventListener}.
     * 
     * @author Chris Banes
     */
    public static interface OnPullEventListener<V extends View> {
         ...
         *        direction the user is pulling. Only useful when <var>state</var> is {@link State#PULL_TO_REFRESH} or
